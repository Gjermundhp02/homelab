...
locals {
  manager_ips = {for vm in range(3) :  "${module.talos_manager[vm].name}" => [for ip in flatten(flatten(module.talos_manager[vm].k8s_vm_ip)) : ip if ip != "127.0.0.1"][0]}
  worker_ips =  {for vm in range(3) : "${module.talos_worker[vm].name}" => [for ip in flatten(flatten(module.talos_worker[vm].k8s_vm_ip)) : ip if ip != "127.0.0.1"][0]}
}
